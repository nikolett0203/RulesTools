...
#' @param display_elements Choose to display intersection data directly on the plot.
...
#' @export
ggvenn_custom <- function(data, columns = NULL,
                   show_elements = FALSE,
                   show_stats = 'cp',
  ...
  # create dataframe to store raw strings
  df <- create_df(venn$names, venn$combos, venn$labels$text)
  
  # create single string from df, formatted with newlines, ready to display
  text_to_display <- paste(mapply(format_string, colnames(df), df[1, ]), collapse = "\n")
  ...
  # add string as caption to plot, justified left, if user specifies to do it
  if (!identical(display_element, FALSE)){
    g <- g + 
      labs(caption = text_to_display) +
      theme(plot.caption = element_text(hjust = 0))  
  }

  
  # print intersections to terminal as well
  print("Rule Intersections:")
  cat(text_to_display)
...
prepare_venn_data <- function(data, columns = NULL,
  ...
  # modified to return specific items in each intersection 
  list(shapes = df_shape, texts = df_text, labels = df_label, segs = df_seg, combos = df_element$text, names = df_element$name)
  
}


  create_df <- function(names, intersects, sets){
    
    # remove outside data (none of the rules data should be outside the intersections)
    names <- head(names, -1)
    intersects <- head(intersects, -1) 
    
    # add intersection symbol for clarity in our caption
    pairs <- c("AB", "BC", "CD", "AC", "AD", "BD")
    replacements <- c("A ∩ B", "B ∩ C", "C ∩ D", "A ∩ C", "A ∩ D", "B ∩ D")  
    
    
    for (i in seq_along(pairs)) {
      names <- gsub(pairs[i], replacements[i], names)
    }
    
    # replace letters with actual set names
    size <- length(sets)
    
    for (i in 1:size) {
      names <- gsub(LETTERS[i], sets[i], names)
    }
