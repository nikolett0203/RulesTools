...
ggvenn <- function(data, columns = NULL,
  ...
  venn$names <- head(venn$names, -1)
  venn$combos <- head(venn$combos, -1)
  

  for (i in seq_along(pairs)) {
    venn$names <- gsub(pairs[i], replacements[i], venn$names)
  }
  
    
  print(venn$names)
  print(venn$labels$text)
  
  print(length(venn$labels$text))
  
  size <- length(venn$labels$text)
  
  for (i in 1:size) {
    venn$names <- gsub(LETTERS[i], venn$labels$text[i], venn$names)
  }
  ...
  df <- as.data.frame(t(venn$combos))
  colnames(df) <- venn$names
  
  # convert to string
  text_to_display <- paste(
    mapply(function(name, combo) {
      paste0(name, ":\n", combo, "\n")
    }, colnames(df), df[1, ]), collapse = "\n"
  )
  print(text_to_display)
  
  g <- g + labs(caption = text_to_display) +
    theme(
